...
public class SocialMediaController {
	...
	@Autowired
	SocialMediaService _socialMediaServices;
	...
	public ResponseEntity<List<SocialMedia>> getSocialMedias(@RequestParam(value="name", required=false) String name) {
		...
		if (name == null) {
			socialMedias = _socialMediaServices.findAllSocialMedias();
			...
			SocialMedia socialMedia = _socialMediaServices.findByName(name);
			if (socialMedia == null) {
	...
	public ResponseEntity<SocialMedia> getSocialMediaById(@PathVariable("id") Long idSocialMedia) {
		...
		SocialMedia socialMedia = _socialMediaServices.findById(idSocialMedia);
		if(socialMedia == null) {
	...
	public ResponseEntity<?> createSocialMedia(@RequestBody SocialMedia socialMedia, UriComponentsBuilder uriComponentsBuilder) {
		...
		if(_socialMediaServices.findByName(socialMedia.getName()) != null) {
			return new ResponseEntity(HttpStatus.NO_CONTENT);
		}
		
		_socialMediaServices.saveSocialMedia(socialMedia);
		SocialMedia sm_created = _socialMediaServices.findByName(socialMedia.getName());
		
		HttpHeaders headers = new HttpHeaders();
		headers.setLocation(
				uriComponentsBuilder.path("/v1/socialMedias/{id}")
				.buildAndExpand(sm_created.getIdSocialMedia())
				.toUri());
	...
	public ResponseEntity<SocialMedia> updateSocialMedia(@PathVariable("id") Long idSocialMedia, @RequestBody SocialMedia socialMedia) {
		...
		SocialMedia currentSocialMedia = _socialMediaServices.findById(idSocialMedia);
		...
		_socialMediaServices.updateSocialMedia(currentSocialMedia);
	...
	public ResponseEntity<?> deleteSocialMedia(@PathVariable("id") Long idSocialMedia) {
		...
		_socialMediaServices.deleteSocialMediaById(idSocialMedia);
